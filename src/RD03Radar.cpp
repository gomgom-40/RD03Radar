...
// ──────────────────────────────────────────────
#define RADAR_INIT_DELAY_MS       3000    // Delay after power-on before init
#define UART_STALE_TIMEOUT_MS     100     // UART buffer stale timeout
#define MAX_BUFFER_SIZE           256     // Max UART buffer size
#define BUFFER_CLEAR_TIMEOUT_MS   30000   // Buffer clear timeout
#define PUBLISH_INTERVAL_MS       40      // Minimum publish interval (ms)
#define FAST_EXIT_TIMEOUT_MS      15000   // Fast exit timeout (15 sec)
#define WATCHDOG_SOFT_RESET_MS    90000   // Soft reset after 90 sec silence
#define WATCHDOG_HARD_RESET_MS    180000  // Hard reset after 180 sec silence
#define LONG_SILENCE_RESET_MS     43200000UL  // 12 hours in ms

// Radar initialization commands (frame format specific to RD-03)
...
// For SoftwareSerial or any Stream (ESP8266, etc.)
...
// For ESP32 HardwareSerial – specify RX/TX pins
bool RD03Radar::begin(int rxPin, int txPin) {
    if (_initialized) return true;
    if (!_isHardwareSerial) return false; // Wrong constructor
...
// For ESP8266 SoftwareSerial – no pins needed (already configured)
bool RD03Radar::begin() {
    if (_initialized) return true;
    if (_isHardwareSerial) return false; // Wrong overload

    // SoftwareSerial already initialized by user
    delay(RADAR_INIT_DELAY_MS);
...
void RD03Radar::manualLightControl(bool turnOn) {
    ...
    if (_lightCallback) {
        _lightCallback(turnOn, turnOn ? "Manual ON" : "Manual OFF");
    }
    updateStatus(RD03Status::OK, turnOn ? "Manual ON" : "Manual OFF");
}
...
const char* RD03Radar::getVersion() { return "1.1.0"; }
const char* RD03Radar::getInfo()    { return "RD03Radar Library v1.1.0 - Ai-Thinker RD-03 mmWave Presence Detection"; }
...
String RD03Radar::processUART() {
    ...
    if (newLineFound && !_uartBuffer.empty()) {
        String line((char*)_uartBuffer.data(), _uartBuffer.size());
        _uartBuffer.clear();
        line.trim();
        line.replace("\r", "");
    ...
    if (_uartBuffer.size() >= MAX_BUFFER_SIZE) {
        _uartBuffer.clear();
        updateStatus(RD03Status::BUFFER_OVERFLOW, "UART buffer overflow - cleared");
    }
...
float RD03Radar::extractDistance(const String& message) {
    ...
    int pos = message.indexOfAny("0123456789", 6);
    ...
    return (endPtr != numStr.c_str() && endPtr != numStr.c_str() + numStr.length()) ? distance : 0.0f;
}
...
void RD03Radar::watchdogCheck() {
    ...
    if (silentMs >= LONG_SILENCE_RESET_MS) {
        updateStatus(RD03Status::WATCHDOG_RESET, "Long silence detected - consider hard reset");
        // In Arduino we can't restart the board here – user should handle ESP.restart() if needed
    }
}
